...
export default class CardBekasi extends Component {
  ...
  componentDidMount() {
    ...
    getDataBekasi()
      .then(response => {
        this.setState({ data: response.Data.slice(0, 4) });
      })
    ...
    const items = this.state.data.map(values => {
      return (
        <View
          ...
          <Text style={{ fontSize: 13, color: '#E5DDDD' }}>{values.kecamatan}</Text>
          <View style={{ flexDirection: 'row' }}>
            <Text style={{ fontSize: 13, color: '#FC7302' }}>{values.terkonfirmasi}</Text>
          </View>
        </View>
      );
    });
